...
export default function TranslatePage() {
  ...
  return (
    <SplashStylePage contentClassName="p-4 space-y-4 pb-24">
      {/* Header */}
        ...
        <div className="grid grid-cols-1 gap-1.5">
          {PRESETS.default.map((p) => (
            <button
              ...
              className="text-left px-3 py-2.5 bg-white rounded-xl shadow-sm border border-gray-100 hover:border-[#FFDE32] transition-colors disabled:opacity-50"
            >
          ...
          {messages.map((msg) => (
            <div
              ...
              className={`rounded-xl p-3 space-y-1 ${
                msg.from === 'foreigner'
                  ? 'bg-[#FFFBEE] border border-[#FFE58A] ml-4'
            ...
            >
              ...
              <p className={`text-base font-medium ${
                msg.from === 'foreigner' ? 'text-[#1A1A1A]' : 'text-gray-700'
              }`}>
                {msg.translated}
              </p>
              {/* Replay button */}
              <button
                ...
                className="text-sm text-gray-400 hover:text-[#1A1A1A] flex items-center gap-1 mt-1"
              >
      ...
      {speaking && (
        <div className="flex items-center justify-center gap-2 py-2">
          <div className="flex gap-1">
            ...
            <span className="w-2 h-2 bg-[#FFB800] rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
            <span className="w-2 h-2 bg-[#FFB800] rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
          </div>
          <span className="text-base text-[#1A1A1A] font-medium">{t('translate.speaking')}</span>
        </div>
      )}
      ...
      <div className="fixed bottom-16 left-0 right-0 max-w-lg mx-auto px-4 pb-2 space-y-2">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-3 bg-white border border-gray-200 rounded-xl text-base shadow-sm focus:outline-none focus:ring-2 focus:ring-[#FFDE32]"
          />
          <button
            onClick={sendCustom}
            disabled={!customText.trim() || processing}
            className="px-4 py-3 bg-[#1A1A1A] text-[#FFDE32] rounded-xl font-medium disabled:bg-gray-300 disabled:text-gray-100 shadow-sm"
          >
            {t('translate.send')}
          </button>
        </div>
