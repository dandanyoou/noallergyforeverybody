...
export default function ResultPage() {
  ...
  if (!state?.result) {
    return (
      <SplashStylePage contentClassName="p-6 text-center space-y-4 pt-20">
        ...
        <button onClick={() => navigate('/')} className="text-[#1A1A1A] font-semibold btn-press">
          {t('result.go_back')}
        </button>
      </SplashStylePage>
    )
  ...
  return (
    <SplashStylePage contentClassName="p-4 space-y-4 pb-6">
      {/* Food Image + Name */}
      <motion.div
      ...
      >
        {hasDanger ? (
          ...
          <div className="bg-gradient-to-r from-[#FFFBEE] to-[#FFF6CC] border border-[#FFE58A] rounded-2xl p-4 flex items-center gap-3">
            <div className="w-9 h-9 bg-[#FFDE32] rounded-xl flex items-center justify-center">
              <svg className="w-6 h-6 text-[#1A1A1A]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <p className="text-[#1A1A1A] font-semibold text-base">{t('result.safe')}</p>
          </div>
        )}
      </motion.div>
        ...
        {result.top3.map((item, i) => (
          <div key={i} className="flex items-center gap-3">
            <span className={`text-sm font-bold w-6 h-6 flex items-center justify-center rounded-md ${
              i === 0 ? 'bg-[#FFDE32] text-[#1A1A1A]' : 'bg-gray-50 text-gray-400'
            }`}>{i + 1}</span>
            <div className="flex-1">
              ...
              <div className="mt-1.5 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                <motion.div
                  className={`h-full rounded-full ${i === 0 ? 'bg-gradient-to-r from-[#FFDE32] to-[#FFB800]' : 'bg-gray-300'}`}
                  initial={{ width: 0 }}
      ...
      {result.claude_analysis && (
        ...
        >
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 bg-gradient-to-br from-[#FFDE32] to-[#FFB800] rounded-md flex items-center justify-center">
              <svg className="w-3.5 h-3.5 text-[#1A1A1A]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
      ...
      <motion.button
        ...
        className="btn-press w-full py-4 rounded-2xl font-bold text-lg text-[#1A1A1A] border-2 border-[#FFE58A] hover:bg-[#FFF6CC] transition-all"
      >
