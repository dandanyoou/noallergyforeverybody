...
const RISK_COLORS = {
  ...
  safe:    { fill: '#FFDE32', border: '#FFB800' },
...
export default function MapPage() {
  ...
  return (
    <SplashStylePage contentClassName="p-4 space-y-3 pb-6">
      {/* Tab */}
        ...
        {(['restaurants', 'hotspots'] as const).map((tb) => (
          <button
            ...
            className={`btn-press flex-1 py-3 rounded-2xl text-base font-semibold transition-all ${
              tab === tb
                ? 'bg-gradient-to-r from-[#FFDE32] to-[#FFB800] text-[#1A1A1A] shadow-sm'
      ...
      {tab === 'restaurants' && (
        <>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={jumpToSeoul}
              className="btn-press ml-auto text-sm text-[#1A1A1A] font-semibold px-3 py-2 bg-[#FFFBEE] rounded-xl hover:bg-[#FFF6CC] transition-colors border border-[#FFE58A]"
            >
              {t('map.go_to_seoul')}
            </button>
          </div>
          ...
          <div className="relative">
            ...
            {showSearchHere && (
              <button
                ...
                disabled={loading}
                className="btn-press absolute top-3 left-1/2 -translate-x-1/2 z-[1000] glass shadow-elevated px-4 py-2.5 rounded-full text-base font-semibold text-[#1A1A1A] border border-[#FFE58A] hover:bg-[#FFFBEE] transition-all"
              >
            ...
            {restaurants.length > 0 && userAllergies.length > 0 && (
              <div className="absolute bottom-3 left-3 z-[1000] glass rounded-xl px-3 py-2 shadow-sm space-y-1">
                ...
                <div className="flex items-center gap-1.5">
                  <span className="w-2.5 h-2.5 rounded-full bg-[#FFDE32]" />
            ...
            <div className="absolute bottom-3 right-3 z-[1000] glass shadow-sm px-3 py-1.5 rounded-xl text-xs text-gray-500 font-medium flex items-center gap-1.5">
              ...
              {!loading && restaurants.length > 0 && (
                <span className={`px-1.5 py-0.5 rounded-md text-[#1A1A1A] font-bold ${source === 'seoul' ? 'bg-[#FFDE32]' : 'bg-[#FFD36A]'}`}>
